...
Item * ObjectManager::addSphere(string name){
    ObjectFactory& objectFactory = ObjectFactory::getInstance();
    ...
    if(result){
        SphereData data = dialog.getData();

        RenderObject * t = objectFactory.createSphere(name,
                                                      data.radius,
                                                      data.rings,
                                                      data.sections);
        this->scene->addRenderObject(t);
        Item* item = sceneTree->addObject(t, RB_SPHERE_TYPE);
        return item;
    }else{
        return NULL;
    }
...
Item * ObjectManager::addCloud(Sphere* sphere){
    ObjectFactory& objectFactory = ObjectFactory::getInstance();
